...
template <typename T>
T tag_combine(T const &lhs, T const &rhs);
template <typename T>
std::string tag_sprint(T const &tag);
template <typename T>
T tag_alloc(unsigned int offset);
...
template <>
uint8_t tag_combine(uint8_t const &lhs, uint8_t const &rhs);
template <>
std::string tag_sprint(uint8_t const &tag);
template <>
uint8_t tag_alloc<uint8_t>(unsigned int offset);
