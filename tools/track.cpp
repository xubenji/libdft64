...
VOID EntryPoint(VOID *v)
{
    for (IMG img = APP_ImgHead(); IMG_Valid(img); img = IMG_Next(img))
    {
        ...
        if (RTN_Valid(test_get_rtn))
        {
            ...
            RTN_InsertCall(test_get_rtn, IPOINT_BEFORE, (AFUNPTR)TestGetHandler, IARG_FUNCARG_ENTRYPOINT_VALUE, 0,
                           IARG_END);
        ...
        if (RTN_Valid(test_set_rtn))
        {
            ...
            RTN_InsertCall(test_set_rtn, IPOINT_BEFORE, (AFUNPTR)TestSetHandler, IARG_FUNCARG_ENTRYPOINT_VALUE, 0,
        ...
        if (RTN_Valid(test_getval_rtn))
        {
            ...
            RTN_InsertCall(test_getval_rtn, IPOINT_BEFORE, (AFUNPTR)TestGetValHandler, IARG_THREAD_ID,
...
int main(int argc, char *argv[])
{
    ...
    {
        std::cerr << "Sth error in PIN_Init. Plz use the right command line options." << std::endl;
        return -1;
    }
